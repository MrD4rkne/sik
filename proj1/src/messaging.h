...
class MessageSender {
  public:
    MessageSender(int socket_fd, sockaddr_in* address, logging::Logger& logger)
        : socket_fd(socket_fd), address(address), logger(logger) {
        if (address == nullptr) {
    ...
    bool send_message(const char* buffer, size_t bytes_to_send) {
        ...
        try {
            ...
            while (total_sent < bytes_to_send) {
                sent_bytes = sendto(socket_fd, buffer + total_sent,
                ...
                logger.logDebug("Sent ", sent_bytes, " of ", bytes_to_send,
                                " bytes.");
            }
...
class hello_message_handler : public MessageHandler {
  public:
    bool handle(Node& node, logging::Logger& logger, const domain::peer& peer,
        ...
        if (!message_sender.send_message(hello_message_response.c_str(),
