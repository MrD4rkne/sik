...
namespace packets {
    ...
    namespace mappers{
        ...
        static inline peer parse_peer(const char* buffer, size_t buffer_size) {
            ...
            peer_address_length_t peer_address_length = (peer_address_length_t)buffer[0];
            ...
            if (buffer_size < peer_address_length + sizeof(port_t)) {
                throw std::runtime_error("Buffer size is too small to parse peer address and port.");
            }
        ...
        static inline void peer_to_network_order(const peer& peer, char* buffer, size_t buffer_size) {
            if (buffer_size < sizeof(peer_address_length_t) + peer.get_address_length() + sizeof(port_t)) {
        ...
        inline std::string create_hello_response_packet(std::vector<peer> peers) {
            ...
            *reinterpret_cast<count_t*>(&packet[sizeof(message_type_t)]) = htons(peers.size());
            ...
            for (const auto& peer : peers) {
                peer_to_network_order(peer, &packet[current_size], packet_size - current_size);
