...
namespace messages{
    ...
    T deserialize_message(const std::string& message) {  
        ...
        if (tokens.back().size() < END_OD_MESSAGE.size() ||
            tokens.back().compare(tokens.back().size() - END_OD_MESSAGE.size(), END_OD_MESSAGE.size(), END_OD_MESSAGE) != 0) {
            throw std::runtime_error("Last word does not end with line ending");
        }

        // Remove the line ending from the last token
        tokens.back().erase(tokens.back().size() - END_OD_MESSAGE.size(), END_OD_MESSAGE.size());
        if (tokens.back().empty()) {
            throw std::runtime_error("Last word is empty after removing line ending");
        }
