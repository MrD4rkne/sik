...
int main(int argc, char* argv[]) {
    ...
    try {
        ...
        port_t port_number = input::parse_input<port_t>(PORT_NUMBER_ARG,args_map.get_value(PORT_NUMBER_ARG), 0, 65535);
        std::cout << "Port number: " << port_number << std::endl;
        uint16_t k = input::parse_input<uint16_t>(K_ARG, args_map.get_value(K_ARG), 1, 100);
        std::cout << "K: " << k << std::endl;
        uint8_t n = input::parse_input<uint8_t>(N_ARG, args_map.get_value(N_ARG), 1, 255);
        std::cout << "N: " << static_cast<int>(n) << std::endl;
        uint32_t m = input::parse_input<uint32_t>(M_ARG, args_map.get_value(M_ARG), 1, 1000000);
