...
class string_parser{
  public:
    ...
    static T parse_numeric(const std::string& s, T min, T max) {
      ...
      if (value < min || value > max) {
        throw std::out_of_range("Must be between " + std::to_string(min) + " and " + std::to_string(max));
      }
  ...
  template<>
  inline unsigned int input::string_parser::parse_numeric<unsigned int>(const std::string& str, unsigned int min, unsigned int max) {
  ...
  template<>
  inline uint8_t input::string_parser::parse_numeric<uint8_t>(const std::string& str, uint8_t min, uint8_t max) {
    return static_cast<uint8_t>(parse_numeric<unsigned int>(str, min, max));
  }

  template<>
  inline unsigned long input::string_parser::parse_numeric<unsigned long>(const std::string& str, unsigned long min, unsigned long max) {
    unsigned long value = std::stoul(str);
    if (value < min || value > max) {
      throw std::out_of_range("Must be between " + std::to_string(min) + " and " + std::to_string(max));
    }
  ...
  template<>
  inline unsigned short input::string_parser::parse_numeric<unsigned short>(const std::string& str, unsigned short min, unsigned short max) {
    unsigned long value = std::stoul(str);
    if (value < min || value > max) {
      throw std::out_of_range("Must be between " + std::to_string(min) + " and " + std::to_string(max));
    }
  ...
  template<typename T>
static T parse_input(const std::string& arg_name, const std::string& val, T min, T max) {
    try{
        return input::string_parser::parse_numeric<T>(val, min, max);
    } catch (const std::invalid_argument& e) {
        throw std::invalid_argument("Invalid argument for " + arg_name + ": " + e.what());
    } catch (const std::out_of_range& e) {
        throw std::out_of_range("Out of range for " + arg_name + ": " + e.what());
    }
