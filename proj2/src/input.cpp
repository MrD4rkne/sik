...
namespace input{
    args_parses_t::args_parses_t(int argc, char* argv[], const std::unordered_map<std::string, arg_t>& allowed_args):
        ...
        for (int i = 1; i < argc; i++) {
            ...
            if (flags.find(current_arg) != flags.end() || value_args.find(current_arg) != value_args.end()) {
                throw std::invalid_argument("Duplicate argument: " + current_arg);
            }

            bool is_flag = it->second.is_flag;
            if (is_flag && (i + 1 >= argc || argv[i + 1][0] == '-')) {
                throw std::invalid_argument("Argument requires a value: " + current_arg);
            }
