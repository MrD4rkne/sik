...
public:
    using delegate_t = std::function<results::Result(const ip::IPAddress&, network::MessageSender&, T&, logging::Logger&, const std::string& message)>;

    void register_handler(const std::string& message_type, delegate_t delegate) {
        handlers[message_type] = std::move(delegate);
    }

    results::Result handle(const std::string& message_type, const ip::IPAddress& ip_addr,
